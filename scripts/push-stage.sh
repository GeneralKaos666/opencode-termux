...
# Filter: skip garbage patterns (auto-generated by CI Pages workflow)
FILTERED=()
for asset in "${ASSETS[@]}"; do
    bname=$(basename "$asset")
    skip=0
    # Skip daily mirrorlist snapshots (keep only -latest)
    case "$bname" in
        hope2333-mirrorlist-1.0.*-*-any.pkg.tar.xz)
            case "$bname" in *latest*) ;; *) skip=1 ;; esac ;;
    esac
    # Skip old db files (CI-generated)
    case "$bname" in
        hope2333.db|hope2333.db.tar.gz|hope2333_final.db|hope2333_final.db.tar.gz) skip=1 ;;
    esac
    # Skip duplicate SHA256SUMS
    case "$bname" in
        SHA256SUMS-*.txt) skip=1 ;;
    esac
    if [ "$skip" -eq 1 ]; then
        echo "  SKIP (garbage): $bname"
        continue
    fi
    FILTERED+=("$asset")
done
...
echo ""
echo "Total assets (after filter): ${#ASSETS[@]}"
echo ""
