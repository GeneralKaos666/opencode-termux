...
包名变更与主线切换已于 27/28 生效：glibc 与 bionic 构建已区分，旧 `opencode` 包已更名为 `opencode-wrapper`，`opencode` 已由 native bionic 线继承并成为稳定主线（正式发布渠道，BETA 预发布阶段已结束）。
...
- `opencode` 与 `opencode-wrapper` 互斥，不可并存安装（同现双轨关系保持不变）。
- 过渡期新增 `opencode-wrapper-standalone`：命令入口为 `opencode-wrapper`、库路径独立（`bin/opencode-wrapper` + `lib/opencode-wrapper`），**与 `opencode` 包可共存**，仅发布一个冻结版本，专用于过渡期回退。
...
3. 同时安装 `opencode-wrapper-standalone` 作为保底回退（入口 `opencode-wrapper`）。

### 打包注意

- `opencode-wrapper-standalone` 包**不得**对字面名 `opencode` 声明 `Conflicts`，避免误伤已继承该名的 native 包。
- 互斥关系仅存在于 `opencode` ↔ `opencode-wrapper` 这一对。
...
```bash
# apt/pkg
dpkg -i /path/to/opencode-wrapper_<version>_aarch64.deb

# pacman
pacman -U /path/to/opencode-wrapper-<version>-1-aarch64.pkg.tar.xz
```
...
dpkg -i opencode-wrapper_<v>_aarch64.deb  # native -> glibc
```
...
**分界裁定**：自 `Push260828` 起，`opencode` 包名 = native bionic 主线；此前 tag 的 `opencode` 资产 = glibc 包装线（现名 opencode-wrapper）。

**红线**：native 包名永远是 `opencode`，不加 `-native` 后缀。

**互斥教义**：三家族（`opencode` native 主线 / `opencode-wrapper` / `opencode-compressed`）互斥不可共存；`opencode-wrapper` 的 deb 现为硬互斥（`Conflicts: opencode, opencode-native, opencode-compressed`；无 `Replaces`）。
...
1. plain `opencode_*` 约 40MB 且场内同现 `opencode-wrapper_*`（约 82MB）或 `opencode-<v>-*-android-native*` 资产 → native 主线时代。
2. plain `opencode_*` 且场内完全无 `opencode-wrapper_*` / `opencode-native_*` / `*-android-native*` 命名资产 → 旧 glibc 时代（更名前）。
3. 大小陷阱：旧 glibc 时代的 plain deb 本身只有 30~41MB，不是 80MB；约 82MB 档是现行 `opencode-wrapper` 家族。单看 plain deb 大小无法区分 1.18.x 的两族，必须看场内是否同现带 `glibc` / `native` 命名的资产。
